# kohya_hires_fix_unified_v2.6.8.py
# SCRIPT_VERSION: 2.6.8
# Изменения:
#   - HiresPreset.__init__: нормализует self.scaler "lanczos"→"lanczos-like"
#     (старые пресеты теперь корректно восстанавливаются в UI dropdown)
...
# Один раз при загрузке проверяем, поддерживает ли текущий PyTorch antialias в F.interpolate
_INTERP_SUPPORTS_ANTIALIAS: bool = "antialias" in _inspect.signature(F.interpolate).parameters
...
INTERP_CHOICES: List[str] = [
    ...
    "bicubic",
    "area",
    "lanczos-like",     # честное название: эмулируется через bicubic + antialias
    "bilinear-antialiased",
    "bicubic-antialiased",
    # "area-antialiased" убран: PyTorch не поддерживает antialias для area
]
...
RESOLUTION_GROUPS = {
    ...
    "Альбом": [(1536, 640), (1344, 768), (1216, 832), (1152, 896), (1536, 1024)],
}
...
def parse_interp(choice: str) -> Tuple[str, bool]:
    ...
    # Один источник истины: lanczos / lanczos-like = честная эмуляция через bicubic + AA
    # Поддерживаем оба варианта: "lanczos" для старых пресетов, "lanczos-like" для нового UI
    if val in {"lanczos", "lanczos-like"}:
        return "bicubic", True
    # antialias для area/nearest не поддерживается PyTorch — сбрасываем
    if val == "area":
        return "area", False
    if val in {"nearest", "nearest-exact"}:
        return val, False
...
def _compute_policy_stops(pairs: List[Tuple[str, float, int]], resolved_pair_mode: str) -> Tuple[float, float]:
    """Вычисляет (first_stop_s, last_stop_s) по заранее отфильтрованным парам.

    pairs — список (tag, s_stop, d_idx), все уже с s_stop > 0 и d_idx не None.
    Возвращает (0.0, 0.0) если пар нет.
    """
    if not pairs:
        return 0.0, 0.0
    stops = [s for _, s, _ in pairs]
    if resolved_pair_mode == "sequential":
        # Порядок пар важен: первая — first_active_interval, последняя — last_active_interval
        return stops[0], stops[-1]
    if resolved_pair_mode in ("d1", "d2"):
        return stops[0], stops[0]
    if resolved_pair_mode == "both_min":
        s = min(stops)
        return s, s
    if resolved_pair_mode == "both_sum":
        s = _clamp(sum(stops), 0.0, 1.0)
        return s, s
    # both / fallback
    return min(stops), max(stops)
...
class HiresPreset:
    def __init__(self, **kwargs: Any) -> None:
        ...
        # Нормализуем устаревшее значение "lanczos" → "lanczos-like"
        if self.scaler == "lanczos":
            self.scaler = "lanczos-like"
...
class Scaler(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs):
        ...
        # Сначала нормализуем режим через parse_interp — один источник истины
        interp_mode, antialias = parse_interp(self.scaler)
        # Страховка для старых пресетов/импортов: area/nearest не поддерживают antialias
        if interp_mode in {"area", "nearest", "nearest-exact"}:
            antialias = False

        # align_corners зависит от реального interp_mode, а не от сырого self.scaler
        # (lanczos → bicubic, bilinear-antialiased → bilinear и т.д. уже учтены выше)
        align_corners = None
        if interp_mode in {"bilinear", "bicubic", "linear", "trilinear"}:
...
class KohyaHiresFix(scripts.Script):
    def __init__(self) -> None:
        ...
        self._denoiser_cb = None  # ссылка на конкретный callback для адресного снятия
        self._last_saved_config = None  # in-memory кеш для ленивого _save_config
    ...
    def _unregister_callback(self) -> None:
        """Адресно снимает только наш denoiser_callback, не трогая остальные."""
        if self._denoiser_cb is not None:
            try:
                script_callbacks.remove_callbacks_for_function(self._denoiser_cb)
            except Exception:
                pass
            self._denoiser_cb = None
        self._cb_registered = False
            
    def _save_config(self) -> None:
        """Автосохранение конфигурации в YAML.

        Использует in-memory кеш последнего сохранённого состояния —
        без чтения файла с диска. Запись происходит только при реальном изменении.
        """
        try:
            new_data = OmegaConf.to_container(self.config, resolve=True)
            # Сравниваем с кешем в памяти, а не с файлом на диске
            if new_data != getattr(self, "_last_saved_config", None):
                _atomic_save_yaml(CONFIG_PATH, new_data)
                self._last_saved_config = new_data
        except Exception as e:
    ...
    def ui(self, is_img2img: bool):
        ...
        # Нормализуем старое значение "lanczos" из сохранённого конфига/пресетов
        if last_scaler == "lanczos":
            last_scaler = "lanczos-like"
        ...
        with gr.Accordion(label="Kohya Hires.fix", open=False):
            with gr.Row():
                ...
                status_indicator = gr.Markdown(
                    f"🟢 **Активен: {last_algo_mode}**" if last_enable else "🔴 **Отключено**",
            ...
            with gr.Group():
                ...
                with gr.Row():
                    ...
                    only_one_pass_enh = gr.Checkbox(
                        ...
                        value=last_only_enh,
                        visible=is_enhanced,
                        info="Enhanced new-logic: после первого активного stop-порога hires.fix больше не вмешивается в блоки."
                    )
            ...
            # Обновление stop_preview_md при изменении s1 / s2 / stop_preview_steps
            def _update_stop_preview_md(enabled, total_steps, s1_v, s2_v):
                if not enabled:
                    return gr.update()
                return gr.update(value=_format_stop_preview_text(total_steps, s1_v, s2_v))

            for _trigger in [s1, s2, stop_preview_steps]:
                _trigger.change(
                    _update_stop_preview_md,
                    inputs=[stop_preview_toggle, stop_preview_steps, s1, s2],
            ...
            btn_refresh_log.click(_refresh_log, outputs=[debug_output])
        ...
        _infotext_map: Dict[str, Any] = {
            ...
            # -- DSHF_ backward-compat: чтение старых PNG (до v2.6.0) --
            # Старые версии скрипта писали только эти 8 базовых параметров.
            # Частичная совместимость: базовые настройки восстановятся,
            # advanced-параметры (smooth, adaptive и т.д.) — нет, т.к. они не писались.
            # Ключи только для ЧТЕНИЯ — в новые PNG не пишутся (используются KHF_*).
            "DSHF_enable":                   enable,
            "DSHF_d1":                       d1,
            "DSHF_d2":                       d2,
            "DSHF_s1":                       s1,
            "DSHF_s2":                       s2,
            "DSHF_scaler":                   scaler,
            "DSHF_downscale":                downscale,
            "DSHF_upscale":                  upscale,
        }
    ...
    def process(
        ...
        # --- PNG Info Save: записываем все параметры в метаданные изображения ---
        # Это позволяет восстановить настройки через "Paste PNG Info".
        # Ключи KHF_* должны совпадать с теми, что зарегистрированы в self.infotext_fields.
        try:
            _ep = p.extra_generation_params
            _ep["KHF_enable"]                 = enable
            _ep["KHF_simple_mode"]            = simple
            _ep["KHF_algo_mode"]              = algo_mode
            _ep["KHF_only_one_pass_enh"]      = only_one_pass_enh
            _ep["KHF_only_one_pass_legacy"]   = only_one_pass_legacy
            _ep["KHF_one_pass_mode"]          = one_pass_mode_select
            _ep["KHF_d1"]                     = d1
            _ep["KHF_d2"]                     = d2
            _ep["KHF_depth_guard"]            = depth_guard
            _ep["KHF_depth_clamp"]            = depth_clamp
            _ep["KHF_depth_pair_mode"]        = depth_pair_mode
            _ep["KHF_s1"]                     = s1
            _ep["KHF_s2"]                     = s2
            _ep["KHF_stop_preview_enabled"]   = stop_preview_enabled
            _ep["KHF_stop_preview_steps"]     = stop_preview_steps
            _ep["KHF_scaler"]                 = scaler
            _ep["KHF_downscale"]              = downscale
            _ep["KHF_upscale"]                = upscale
            _ep["KHF_smooth_scaling_enh"]     = smooth_scaling_enh
            _ep["KHF_smooth_scaling_legacy"]  = smooth_scaling_legacy
            _ep["KHF_smoothing_mode"]         = smoothing_mode_select
            _ep["KHF_smoothing_curve"]        = smoothing_curve
            _ep["KHF_smoothing_curve_param"]  = smoothing_curve_param
            _ep["KHF_early_out"]              = early_out
            _ep["KHF_keep_unitary_product"]   = keep_unitary_product
            _ep["KHF_align_corners_mode"]     = align_ui
            _ep["KHF_recompute_scale_factor"] = recompute_ui
            _ep["KHF_size_rounding_mode"]     = size_rounding_mode
            _ep["KHF_resolution_choice"]      = res_choice
            _ep["KHF_apply_resolution"]       = apply_res
            _ep["KHF_adaptive_by_resolution"] = adapt
            _ep["KHF_adaptive_profile"]       = adapt_prof
            _ep["KHF_legacy_force_align_false"] = legacy_force_align_false
            _ep["KHF_enhanced_safe_clamp"]    = enhanced_safe_clamp
            _ep["KHF_use_old_float_math"]     = use_old_float_math
            _ep["KHF_use_old_onepass_logic"]  = use_old_onepass_logic
            _ep["KHF_debug_mode"]             = debug_mode_val
        except Exception:
            pass
        ...
        d1_idx = int(use_d1) - 1
        d2_idx = int(use_d2) - 1
        # Нормализуем старые значения scaler из пресетов/импортов
        scaler_mode = "lanczos-like" if str(scaler).strip().lower() == "lanczos" else str(scaler)
        ...
        # --- One-pass policy (вычисляется один раз до регистрации callback) ---
        def _resolve_onepass_policy(mode_select: str, use_old: bool) -> str:
            """Определяет политику one-pass с учётом dropdown one_pass_mode_select.

            Режимы dropdown:
              'Авто (по алгоритму)'      — зависит от use_old_onepass_logic
              'Использовать Enhanced'    → first_active_interval (new-logic)
              'Использовать Legacy old'  → legacy_guard (old-logic)
            """
            sel = (mode_select or "").strip().lower()
            if "legacy" in sel:
                return "legacy_guard"
            if "enhanced" in sel:
                return "first_active_interval"
            # Авто: смотрим на use_old_onepass_logic
            if use_old:
                return "legacy_guard"
            return "first_active_interval"

        onepass_policy = _resolve_onepass_policy(one_pass_mode_select, use_old_onepass_logic)

        # --- Static pairs: строятся один раз, все входы уже финальны ---
        # (после adaptive, depth guard swap, normalize — всё известно)
        static_pairs: List[Tuple[str, float, int]] = []
        if resolved_pair_mode in ("both", "sequential", "d1", "d2"):
            if resolved_pair_mode in ("both", "sequential", "d1") and d1_idx is not None and use_s1 > 0:
                static_pairs.append(("p1", use_s1, d1_idx))
            if resolved_pair_mode in ("both", "sequential", "d2") and d2_idx is not None and use_s2 > 0:
                static_pairs.append(("p2", use_s2, d2_idx))
        else:
            # fallback — как both
            if d1_idx is not None and use_s1 > 0:
                static_pairs.append(("p1", use_s1, d1_idx))
            if d2_idx is not None and use_s2 > 0:
                static_pairs.append(("p2", use_s2, d2_idx))

        first_stop_s, last_stop_s = _compute_policy_stops(static_pairs, resolved_pair_mode)
        chosen_stop_s = first_stop_s if onepass_policy == "first_active_interval" else last_stop_s
        
        def _log_mapping(idx, tag: str) -> None:
            """Логирует маппинг depth-индекса в output — только для debug, не влияет на логику."""
            if idx is None:
        ...
        _log_mapping(d1_idx, "d1")
        _log_mapping(d2_idx, "d2")
        ...
        def denoiser_callback(params: script_callbacks.CFGDenoiserParams):
            ...
            try:
                ...
                # Применяем пары из заранее вычисленных static_pairs
                if resolved_pair_mode == "sequential":
                    # Режим "как olds.py": обрабатываем по очереди, после первой успешной — break
                    for tag, s_stop, d_i in static_pairs:
                    ...
                    for _, s_stop, d_i in static_pairs:
                ...
                if use_one:
                    if use_old_onepass_logic:
                        ...
                        # NEW WAY: chosen_stop_s вычислен заранее по onepass_policy
                        if chosen_stop_s > 0 and current >= total * chosen_stop_s:
                            self.step_limit = 1
                ...
                # Адресно снимаем только наш callback
                self._unregister_callback()
        
        # 🔗 РЕГИСТРАЦИЯ CALLBACK'А
        if not self._cb_registered:
            self._denoiser_cb = denoiser_callback
